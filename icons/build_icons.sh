...
[[ -z "${AV_LOGO_PATH}" ]] && AV_LOGO_PATH="${SRC_PREFIX}av_logo.png"
...
if ! declare -F load_linux_png &>/dev/null; then
  load_linux_png() {
    if [[ ! -f "${AV_LOGO_PATH}" ]]; then
      echo "av_logo.png not found at ${AV_LOGO_PATH}" >&2
      exit 1
    fi
    convert "${AV_LOGO_PATH}" -resize 1024x1024 "$1"
  }
fi
...
if ! declare -F load_windows_ico &>/dev/null; then
  load_windows_ico() {
    ...
    convert "${AV_LOGO_PATH}" -resize 256x256 -define icon:auto-resize=256,128,96,64,48,32,24,20,16 "$1"
  }
fi
...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    ...
    # Use av_logo.png directly instead of SVG
    if [[ "$1" == "no-template" ]]; then
      convert "${AV_LOGO_PATH}" -resize 1024x1024 "code_1024.png"
    else
      convert "${AV_LOGO_PATH}" -resize 655x655 "code_logo.png"
...
build_darwin_types() { # {{{
  # Use av_logo.png instead of SVG
  convert "${AV_LOGO_PATH}" -resize 128x128 "code_logo.png"
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    ...
    # Convert av_logo.png to base64 and embed in SVG
    local base64_logo
    base64_logo=$(base64 -w 0 "${AV_LOGO_PATH}" 2>/dev/null || base64 "${AV_LOGO_PATH}" | tr -d '\n')

    cat > "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" << EOF
<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1024" height="1024" viewBox="0 0 1024 1024">
  <image width="1024" height="1024" xlink:href="data:image/png;base64,${base64_logo}"/>
</svg>
EOF
  fi
} # }}}
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    # Use av_logo.png instead of SVG
    convert "${AV_LOGO_PATH}" -resize "${LOGO_SIZE}x${LOGO_SIZE}" "code_logo.png"
...
build_windows_types() { # {{{
  ...
  # Use av_logo.png instead of SVG
  convert "${AV_LOGO_PATH}" -resize 64x64 -background "#F5F6F7" -flatten "code_logo.png"
