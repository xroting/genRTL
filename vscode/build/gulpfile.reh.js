...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(json({ commit, date: readISODate('out-build'), version, serverDownloadUrlTemplate: 'https://github.com/VSCodium/vscodium/releases/download/1.106.38350/averilog-reh-${os}-${arch}-1.106.38350.tar.gz' }))
			.pipe(es.through(function (file) {
