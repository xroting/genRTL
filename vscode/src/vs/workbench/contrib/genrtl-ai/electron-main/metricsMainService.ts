...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	private get oldId() {
		...
		const newKey = 'genrtl-ai.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('genrtl-ai.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('genrtl-ai.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('genrtl-ai.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('genrtl-ai.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		console.log('User is opted out of basic genRTL metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('genRTL posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
