...
import { IgenrtlUpdateService } from '../common/genrtlUpdateService.js';
import { genRTLCheckUpdateRespose } from '../common/genrtlUpdateServiceTypes.js';



export class genRTLMainUpdateService extends Disposable implements IgenrtlUpdateService {
	...
	async check(explicit: boolean): Promise<genRTLCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart genRTL to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<genRTLCheckUpdateRespose> {
		try {
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of genRTL is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'genRTL is up-to-date!'
					}
				}
				else {
