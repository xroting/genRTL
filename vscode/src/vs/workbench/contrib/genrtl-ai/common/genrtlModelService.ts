...
type genrtlModelType = {
	model: ITextModel | null;
	editorModel: IResolvedTextEditorModel | null;
};

export interface IgenrtlModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): genrtlModelType;
	getModelFromFsPath(fsPath: string): genrtlModelType;
	getModelSafe(uri: URI): Promise<genrtlModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IgenrtlModelService = createDecorator<IgenrtlModelService>('voidgenrtlModelService');

class genrtlModelService extends Disposable implements IgenrtlModelService {
	_serviceBrand: undefined;
	static readonly ID = 'voidgenrtlModelService';
	...
	saveModel = async (uri: URI) => {
		await this._textFileService.save(uri, { // we want [our change] -> [save] so it's all treated as one change.
			skipSaveParticipants: true // agenRTL triggering extensions etc (if they reformat the page, it will add another item to the undo stack)
		})
	}
	...
	getModelSafe = async (uri: URI): Promise<genrtlModelType> => {
...
registerSingleton(IgenrtlModelService, genrtlModelService, InstantiationType.Eager);
