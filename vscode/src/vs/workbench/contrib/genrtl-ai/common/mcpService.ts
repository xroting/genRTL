...
import { MCPUserStateOfName } from './genrtlSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('genrtl-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.genrtlSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.genrtlSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.genrtlSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.genrtlSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in genRTL AI Settings
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		this._setMCPServerState(serverName, { status: 'loading', tools: [] })

		await this.genrtlSettingsService.setMCPServerState(serverName, { isOn });
