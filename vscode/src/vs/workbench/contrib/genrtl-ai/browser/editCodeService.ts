...
import { FeatureName } from '../common/genrtlSettingsTypes.js';
import { IgenrtlModelService } from '../common/genrtlModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, genRTLFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IgenrtlModelService private readonly _genrtlModelService: IgenrtlModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._genrtlModelService.initializeModel(model.uri)
	...
	// 		message: `genRTL Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'genrtl-ai.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open genRTL AI's Settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(GENRTL_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'genrtl-ai-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'genrtl-ai-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'genrtl-ai-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'genrtl-ai-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'genrtl-ai-redBG'
	...
	private _getCurrentVoidFileSnapshot = (uri: URI): genRTLFileSnapshot => {
		const { model } = this._genrtlModelService.getModel(uri)
	...
	private _restoreVoidFileSnapshot = async (uri: URI, snapshot: genRTLFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._genrtlModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._genrtlModelService.initializeModel(uri)
		await this._genrtlModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = genRTLPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
					...
					for (let blockNum = currStreamingBlockNum; blockNum < blocks.length; blockNum += 1) {
						...
						if (!(blockNum in addedTrackingZoneOfBlockNum)) {
							...
							if (typeof originalBounds === 'string' || hasOverlap) {
								...
								const retryMsg = 'All of your previous outputs have been ignored. Please re-output ALL SEARCH/REPLACE blocks starting from the first one, and agenRTL the error this time.'
								messages.push(
	...
	// called on genrtl-ai.acceptDiff
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`genRTL error: ${diff}.type not recognized`)
		}
	...
	// called on genrtl-ai.rejectDiff
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IgenrtlCommandBarService private readonly _genrtlCommandBarService: IgenrtlCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(GENRTL_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(GENRTL_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._genrtlCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._genrtlCommandBarService.onDidChangeState(e => {
