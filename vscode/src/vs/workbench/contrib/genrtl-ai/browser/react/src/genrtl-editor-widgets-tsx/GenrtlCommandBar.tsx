...
import { genrtlCommandBarProps } from '../../../genrtlCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	GENRTL_GOTO_NEXT_DIFF_ACTION_ID,
	GENRTL_GOTO_PREV_DIFF_ACTION_ID,
	GENRTL_GOTO_NEXT_URI_ACTION_ID,
	GENRTL_GOTO_PREV_URI_ACTION_ID,
	GENRTL_ACCEPT_FILE_ACTION_ID,
	GENRTL_REJECT_FILE_ACTION_ID,
	GENRTL_ACCEPT_ALL_DIFFS_ACTION_ID,
	GENRTL_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';
...
export const GenrtlCommandBar = ({ uri, editor }: genrtlCommandBarProps) => {
	...
	const commandBarService = accessor.get('IgenrtlCommandBarService')
	const genrtlModelService = accessor.get('IgenrtlModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(GENRTL_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(GENRTL_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(GENRTL_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(GENRTL_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(GENRTL_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(GENRTL_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(GENRTL_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(GENRTL_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-genrtl-ai-bg-2 shadow-md border border-genrtl-ai-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-genrtl-ai-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-genrtl-ai-bg-2 rounded shadow-md border border-genrtl-ai-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-genrtl-ai-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-genrtl-ai-bg-2 rounded shadow-md border border-genrtl-ai-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-genrtl-ai-border-2 [&>*:last-child]:border-r-0">
