...
export const GenRTLInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	const adjustHeight = useCallback(() => {
		...
		const newHeight = Math.min(h + 1, 500) // plus one to agenRTL scrollbar appearing when it shouldn't
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-genrtl-ai-fg-1 placeholder:text-genrtl-ai-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-genrtl-ai-border-3 bg-genrtl-ai-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-genrtl-ai-fg-1 bg-genrtl-ai-bg-2-alt border-b border-genrtl-ai-border-3 sticky top-0 bg-genrtl-ai-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-genrtl-ai-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-genrtl-ai-bg-2-alt text-genrtl-ai-fg-1'}
										`}
...
export const GenRTLSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	...
	return (
		<input
			ref={inputRef}
			defaultValue={value} // Use defaultValue instead of value to agenRTL recreation
			onChange={handleChange}
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-genrtl-ai-bg-1 text-genrtl-ai-fg-1 placeholder:text-genrtl-ai-fg-3 border border-genrtl-ai-border-2 focus:border-genrtl-ai-border-1
...
export const GenRTLInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	...
	return <WidgetComponent
		className='
			bg-genrtl-ai-bg-1
			@@genrtl-ai-force-child-placeholder-genrtl-ai-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-genrtl-ai-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
						...
						<div
							className={`absolute left-0 ${size === 'xxs' ? 'h-0.5' :
								size === 'xs' ? 'h-1' :
									size === 'sm' ? 'h-1.5' :
										size === 'sm+' ? 'h-2' : 'h-2.5'
								} bg-genrtl-ai-fg-1 rounded-full`}
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-genrtl-ai-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-genrtl-ai-fg-1`}
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-genrtl-ai-bg-1 border-genrtl-ai-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _VoidSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!genrtl-ai-text-genrtl-ai-fg-3
			@@[&_select]:!genrtl-ai-text-xs
			!text-genrtl-ai-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-genrtl-ai-bg-3'>
		<WidgetComponent
...
// export const genRTLScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const genRTLSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => void;
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-genrtl-ai-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
...
export const GenRTLDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-genrtl-ai-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-genrtl-ai-fg-4 text-xs mb-1 px-1">
