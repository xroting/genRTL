...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-genrtl-ai-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IgenrtlSettingsService')
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-genrtl-ai-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-genrtl-ai-fg-3 hover:text-genrtl-ai-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-genrtl-ai-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with genRTL, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by genrtl-ai.`
							: `genRTL recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
				...
				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-genrtl-ai-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-genrtl-ai-border-2 bg-genrtl-ai-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-genrtl-ai-fg-3' : ''}`}
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='genrtl-ai-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='genrtl-ai-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-genrtl-ai-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<GenRTLCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-genrtl-ai-bg-1 text-genrtl-ai-fg-1 placeholder:text-genrtl-ai-fg-3 border border-genrtl-ai-border-2 focus:border-genrtl-ai-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-genrtl-ai-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const handleChangeValue = useCallback((newVal: string) => {
		genrtlSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [genrtlSettingsService, providerName, settingName]);
...
// 	const needsModel = isProviderNameDisabled(providerName, genrtlSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const genrtlSettingsService = accessor.get('IgenrtlSettingsService')

// 	// const { enabled } = genrtlSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = genrtlSettingsService.state.settingsOfProvider[providerName].enabled
// 						genrtlSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [genrtlSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const genRTLProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = genrtlSettingsState.globalSettings[settingName]
	...
	return <ButtonLeftTextRightOption
		leftButton={<GenRTLSwitch
			...
			value={enabled}
			onChange={(newVal) => {
				genrtlSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <GenRTLInputBox2
		...
		onChangeText={(newText) => {
			genrtlSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		genrtlSettingsService.setGlobalSetting('enableFastApply', newVal)
	...
	return <GenRTLCustomDropdownBox
		...
		options={options}
		selectedOption={genrtlSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-genrtl-ai-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='genrtl-ai-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`genRTL automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const genrtlSettingsService = accessor.get('IgenrtlSettingsService')
	return <div
		className={`text-genrtl-ai-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { genrtlSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		genrtlSettingsService.setGlobalSetting('autoApprove', {
			...genrtlSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [genrtlSettingsService, metricsService])
	...
	return <>
		...
		/>
		<span className="text-genrtl-ai-fg-3 text-xs">{desc}</span>
	</>
}
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const genrtlSettings = useSettingsState()
	const isOn = genrtlSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-genrtl-ai-border-2 bg-genrtl-ai-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-genrtl-ai-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-genrtl-ai-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-genrtl-ai-bg-2 text-genrtl-ai-fg-3 rounded-sm text-xs"

									data-tooltip-id='genrtl-ai-tooltip'
									data-tooltip-content={tool.description || ''}
									data-tooltip-class-name='genrtl-ai-max-w-[300px]'
								>
									{removeUniquePrefix(tool.name)}
								</span>
							))
						) : (
							<span className="text-xs text-genrtl-ai-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-genrtl-ai-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-genrtl-ai-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-genrtl-ai-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-genrtl-ai-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'genrtl-ai-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(genrtlSettingsService.state, null, 2)
			downloadName = 'genrtl-ai-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					genrtlSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@genrtl-ai-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-genrtl-ai-bg-2 hover:bg-genrtl-ai-bg-2/80 text-genrtl-ai-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`genRTL AI's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-genrtl-ai-fg-3 mb-2`}>{`genRTL can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-genrtl-ai-fg-3 mb-2`}>{`genRTL can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-genrtl-ai-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
														...
														data-tooltip-class-name='genrtl-ai-max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<GenRTLSwitch
																...
																onChange={(newVal) => genrtlSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-genrtl-ai-fg-3 bg-genrtl-ai-bg-1 border border-genrtl-ai-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-genrtl-ai-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<GenRTLSwitch
															...
															onChange={(newVal) => genrtlSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-genrtl-ai-fg-3 bg-genrtl-ai-bg-1 border border-genrtl-ai-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-genrtl-ai-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<GenRTLSwitch
															...
															onChange={(newVal) => genrtlSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<GenRTLSwitch
															...
															onChange={(newVal) => genrtlSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-genrtl-ai-fg-3 mt-1'>{`Settings that control the visibility of genRTL suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<GenRTLSwitch
															...
															onChange={(newVal) => genrtlSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-genrtl-ai-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<GenRTLSwitch
															...
															onChange={(newVal) => genrtlSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-genrtl-ai-fg-3 bg-genrtl-ai-bg-1 border border-genrtl-ai-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-genrtl-ai-fg-3 mb-4'>{`Transfer your editor settings into genrtl-ai.`}</h4>
								...
								<div>
									...
									<h4 className='text-genrtl-ai-fg-3 mb-4'>{`Transfer genRTL AI's Settings and chats in and out of genrtl-ai.`}</h4>
								...
								<div>
									...
									<h4 className={`text-genrtl-ai-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-genrtl-ai-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep genRTL running smoothly. You may opt out below. Regardless of this setting, genRTL never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-genrtl-ai-fg-3 mb-4`}>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<GenRTLSwitch
													...
													onChange={(newValue) => {
														genrtlSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
												<span className='text-genrtl-ai-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-genrtl-ai-fg-3 text-xs mt-1'>
											{`When disabled, genRTL will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
