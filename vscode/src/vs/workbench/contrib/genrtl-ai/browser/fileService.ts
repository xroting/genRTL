...
class FilePromptActionService extends Action2 {
	private static readonly GENRTL_COPY_FILE_PROMPT_ID = 'genrtl-ai.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.GENRTL_COPY_FILE_PROMPT_ID,
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await genrtlModelService.getModelSafe(uri)).model?.getLanguageId() || '',
