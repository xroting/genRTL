...
import { GENRTL_VIEW_CONTAINER_ID, GENRTL_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { GENRTL_TOGGLE_SETTINGS_ACTION_ID } from './genrtlSettingsPane.js';
...
const GENRTL_OPEN_SIDEBAR_ACTION_ID = 'genrtl-ai.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: GENRTL_OPEN_SIDEBAR_ACTION_ID, title: localize2('voidOpenSidebar', 'Void: Open Sidebar'), f1: true });
	}
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(GENRTL_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(GENRTL_OPEN_SIDEBAR_ACTION_ID)
		}
...
const GENRTL_CMD_SHIFT_L_ACTION_ID = 'genrtl-ai.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', GENRTL_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(GENRTL_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'genrtl-ai.settingsAction',
			title: `genRTL AI's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(GENRTL_TOGGLE_SETTINGS_ACTION_ID)
	}
})
