...
import { IgenrtlSCMService } from '../common/genrtlSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IgenrtlSettingsService } from '../common/genrtlSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/genrtlSettingsTypes.js'
...
const loadingContextKey = 'genrtlSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private genrtlSCM: IgenrtlSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IgenrtlSettingsService private readonly genrtlSettingsService: IgenrtlSettingsService,
		...
		this.genrtlSCM = ProxyChannel.toService<IgenrtlSCMService>(mainProcessService.getChannel('genrtl-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.genrtlSCM.gitStat(path),
					this.genrtlSCM.gitSampledDiffs(path),
					this.genrtlSCM.gitBranch(path),
					this.genrtlSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.genrtlSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.genrtlSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.genrtlSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'genrtlSCM - Commit Message' },
			})
		})
	}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'genrtl-ai.generateCommitMessageAction',
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'genrtl-ai.loadingGenerateCommitMessageAction',
