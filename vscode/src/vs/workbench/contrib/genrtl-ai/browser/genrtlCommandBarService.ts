...
export class genrtlCommandBarService extends Disposable implements IgenrtlCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'genrtl-ai.genrtlCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._genrtlModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IgenrtlCommandBarService, genrtlCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type genrtlCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountgenrtlCommandBar(root, accessor, { uri, editor } satisfies genrtlCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies genrtlCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_ACCEPT_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_REJECT_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_GOTO_PREV_URI_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_ACCEPT_FILE_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_REJECT_FILE_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: GENRTL_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
