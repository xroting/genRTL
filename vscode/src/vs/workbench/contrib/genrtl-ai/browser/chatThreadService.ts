...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/genrtlSettingsTypes.js';
...
import { genRTLFileSnapshot } from '../common/editCodeServiceTypes.js';
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	// should probably re-use code from genRTL/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart genRTL)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart genRTL), cancel that last tool
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const genRTLFileSnapshotOfURI: { [fsPath: string]: genRTLFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._genrtlModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { genRTLFileSnapshot: oldVoidFileSnapshot } = res
		...
		return { genRTLFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { genRTLFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { genRTLFileSnapshotOfURI: genRTLFileSnapshotOfURI ?? {}, },
		})
	}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._genrtlModelService.getModelSafe(uri)
