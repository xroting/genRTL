...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  setpath "product" "nameShort" "genRTL - Insiders"
  setpath "product" "nameLong" "genRTL - Insiders"
  setpath "product" "applicationName" "genRTL-insiders"
  setpath "product" "dataFolderName" ".genRTL-insiders"
  setpath "product" "linuxIconName" "genRTL-insiders"
  setpath "product" "quality" "insider"
  setpath "product" "urlProtocol" "genRTL-insiders"
  setpath "product" "serverApplicationName" "genRTL-server-insiders"
  setpath "product" "serverDataFolderName" ".genRTL-server-insiders"
  setpath "product" "darwinBundleIdentifier" "com.genRTL.insiders"
  setpath "product" "win32AppUserModelId" "genRTL.genRTLInsiders"
  setpath "product" "win32DirName" "genRTL Insiders"
  setpath "product" "win32MutexName" "genRTLinsiders"
  setpath "product" "win32NameVersion" "genRTL Insiders"
  setpath "product" "win32RegValueName" "genRTLInsiders"
  setpath "product" "win32ShellNameShort" "genRTL Insiders"
  ...
  setpath "product" "tunnelApplicationName" "genRTL-insiders-tunnel"
  setpath "product" "win32TunnelServiceMutex" "genRTLinsiders-tunnelservice"
  setpath "product" "win32TunnelMutex" "genRTLinsiders-tunnel"
  ...
  setpath "product" "nameShort" "genRTL"
  setpath "product" "nameLong" "genRTL"
  setpath "product" "applicationName" "genRTL"
  setpath "product" "linuxIconName" "genRTL"
  setpath "product" "quality" "stable"
  setpath "product" "urlProtocol" "genRTL"
  setpath "product" "serverApplicationName" "genRTL-server"
  setpath "product" "serverDataFolderName" ".genRTL-server"
  setpath "product" "darwinBundleIdentifier" "com.genRTL"
  setpath "product" "win32AppUserModelId" "genRTL.genRTL"
  setpath "product" "win32DirName" "genRTL"
  setpath "product" "win32MutexName" "genRTL"
  setpath "product" "win32NameVersion" "genRTL"
  setpath "product" "win32RegValueName" "genRTL"
  setpath "product" "win32ShellNameShort" "genRTL"
  ...
  setpath "product" "tunnelApplicationName" "genRTL-tunnel"
  setpath "product" "win32TunnelServiceMutex" "genRTL-tunnelservice"
  setpath "product" "win32TunnelMutex" "genRTL-tunnel"
...
# Skip patch application if SKIP_SOURCE is yes (patches already applied)
if [[ "${SKIP_SOURCE}" != "yes" ]]; then
  ...
  echo "Skipping patch application (SKIP_SOURCE=yes, patches already applied)"
fi
# }}}
...
# Skip npm install if SKIP_NPM_INSTALL is set and node_modules exists
if [[ "${SKIP_NPM_INSTALL}" == "yes" && -d "node_modules" ]]; then
  echo "Skipping npm install (SKIP_NPM_INSTALL=yes and node_modules exists)"
else
...
# Replace all logo/icon files with genRTL logo design
echo "Replacing logo files with genRTL design..."

# Define the genRTL SVG content (embedded from av_logo.png)
# Note: The SVG will be generated by convert_logo.py script
# Copy av_logo.png based SVG to code-icon.svg locations
if [[ -f "../av_logo.png" ]]; then
...
import base64
import os
with open('../av_logo.png', 'rb') as f:
    logo_data = base64.b64encode(f.read()).decode('utf-8')
svg_content = f'''<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" viewBox=\"0 0 100 100\" width=\"1024\" height=\"1024\">
  <image width=\"100\" height=\"100\" xlink:href=\"data:image/png;base64,{logo_data}\"/>
</svg>'''
with open('src/vs/workbench/browser/media/code-icon.svg', 'w') as f:
    f.write(svg_content)
with open('extensions/github-authentication/media/code-icon.svg', 'w') as f:
    f.write(svg_content)
print('SVG files updated with embedded PNG logo')
"
fi

# Copy av_logo.png to resources for Windows icons
if [[ -f "../av_logo.png" ]]; then
  cp "../av_logo.png" resources/win32/code_70x70.png
  cp "../av_logo.png" resources/win32/code_150x150.png
fi
