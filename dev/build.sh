...
export APP_NAME="genRTL"
export ASSETS_REPOSITORY="VSCodium/vscodium"
export BINARY_NAME="genRTL"
export CI_BUILD="no"
...
export SKIP_NPM_INSTALL="yes"

while getopts ":ilnops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="VSCodium/vscodium-insiders"
      export BINARY_NAME="genRTL-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  if [[ "${SKIP_SOURCE}" != "no" ]]; then
    ...
    # Keep local modifications (logo changes etc.)
    # git add .
